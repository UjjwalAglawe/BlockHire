...
function Sidebar() {
    ...
    return (
        <div className="h-screen  p-6 border-r border-gray-300 bg-white shadow-lg ">
            {/* <h1 className="text-3xl font-extrabold mb-8 text-gray-800">BlockHire</h1> */}
            <ul className="space-y-6">
                {sidebar_item.map((item, index) => (
                    <li key={index} {...(item.name === "Logout" ? { onClick: item.onClick } : {})}>
                        {client == 'client' & item.name == 'Projects' ? null : <NavLink
                            to={item.to}
                            className={({ isActive }) =>
                ...
                <li className='items-center'>
                    ...
                    <button
                        className="flex items-center space-x-2 px-12 py-4 bg-red-600 text-white rounded-lg hover:bg-red-700 focus:outline-none"
